...
describe('core/controller-decorator/test/AgentController.test.ts', () => {
  ...
  describe('context index', () => {
    it('should not set contextIndex on any method', () => {
      const methods = [
        'createThread', 'getThread', 'asyncRun', 'streamRun',
        'syncRun', 'getRun', 'cancelRun',
      ];
    ...
    for (const { name, args } of stubMethods) {
      it(`should throw for unimplemented ${name}`, async () => {
        ...
        await assert.rejects(
          () => instance[name](...args),
          new RegExp(`${name} not implemented`),
        );
      });
    }
  });
  ...
  describe('HTTPControllerMetaBuilder integration', () => {
    ...
    it('should produce correct route metadata for each method', () => {
      ...
      assert.deepStrictEqual(getThread.paramMap, new Map([
      ...
      assert.deepStrictEqual(asyncRun.paramMap, new Map([
        [ 0, new BodyParamMeta() ],
      ]));
      ...
      assert.deepStrictEqual(streamRun.paramMap, new Map([
        [ 0, new BodyParamMeta() ],
      ]));
      ...
      assert.deepStrictEqual(syncRun.paramMap, new Map([
        [ 0, new BodyParamMeta() ],
      ]));
      ...
      assert.deepStrictEqual(getRun.paramMap, new Map([
        [ 0, new PathParamMeta('id') ],
      ]));
      ...
      assert.deepStrictEqual(cancelRun.paramMap, new Map([
        [ 0, new PathParamMeta('id') ],
      ]));
    });
